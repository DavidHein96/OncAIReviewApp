...
// --------------------------------------------------------------------------- //
// gatherEvidence — flatten evidence_verbatim snippets for highlighting
// --------------------------------------------------------------------------- //

const EVIDENCE_SPEC = {
  fields: [
    { name: "evidence", control: "evidence_verbatim" },
    { name: "histology", control: "enum" },
  ],
};

test("gatherEvidence flattens snippets and splits legacy ' ||| ' joins", () => {
  const ev = { fields: { evidence: ["a quote", "b ||| c"], histology: "ignored" } };
  assert.deepEqual(gatherEvidence(ev, EVIDENCE_SPEC), ["a quote", "b", "c"]);
});

test("gatherEvidence returns nothing when there is no evidence field", () => {
  assert.deepEqual(gatherEvidence({ fields: {} }, EVIDENCE_SPEC), []);
});
